...
- (void)setLoadingView:(LoadingView1 *)loadingView{
...
- (LoadingView1 *)loadingView{
    return objc_getAssociatedObject(self, &LoadingViewKey);
}

- (void)beginLoading{
    for (UIView *aView in [self.blankPageContainer subviews]) {
        if ([aView isKindOfClass:[BlankPageView1 class]] && !aView.hidden) {
            return;
        }
    }
    
    if (!self.loadingView) { //初始化LoadingView
        self.loadingView = [[LoadingView1 alloc] initWithFrame:self.bounds];
    }
...
- (void)setBlankPageView:(BlankPageView1 *)blankPageView{
...
- (BlankPageView1 *)blankPageView{
    return objc_getAssociatedObject(self, &BlankPageViewKey);
}

- (void)configBlankPage:(kBlankPageType1)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError reloadButtonBlock:(void (^)(id))block{
...
- (void)configBlankPage:(kBlankPageType1)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError offsetY:(CGFloat)offsetY reloadButtonBlock:(void(^)(id sender))block{
    if (hasData) {
        ...
        if (!self.blankPageView) {
            self.blankPageView = [[BlankPageView1 alloc] initWithFrame:self.bounds];
        }
...
- (void)configWithType:(kBlankPageType1)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError offsetY:(CGFloat)offsetY reloadButtonBlock:(void (^)(id))block{
