...
@class LoadingView1, BlankPageView1;

typedef NS_ENUM(NSInteger, kBlankPageType1)
{
    kBlankPageType1View = 0,
    kBlankPageType1Network,
    kBlankPageType1Address,
    kBlankPageType1ShopCart,
    kBlankPageType1Collection,
    kBlankPageType1Comment,
    kBlankPageType1Coupon,
    kBlankPageType1History,
    kBlankPageType1Message,
    kBlankPageType1Order,
    kBlankPageType1Ohter,
    kBlankPageType1Search,
    
};

typedef NS_ENUM(NSInteger, BadgePositionType1) {
    
    BadgePositionType1Default = 0,
    BadgePositionType1Middle
};
...
- (void)addBadgePoint:(NSInteger)pointRadius withPosition:(BadgePositionType1)type;
...
@property (strong, nonatomic) LoadingView1 *loadingView;
- (void)beginLoading;
- (void)endLoading;


#pragma mark BlankPageView
@property (strong, nonatomic) BlankPageView1 *blankPageView;
- (void)configBlankPage:(kBlankPageType1)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError reloadButtonBlock:(void(^)(id sender))block;
- (void)configBlankPage:(kBlankPageType1)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError offsetY:(CGFloat)offsetY reloadButtonBlock:(void(^)(id sender))block;
@end

@interface LoadingView1 : UIView
...
@interface BlankPageView1 : UIView
@property (strong, nonatomic) UIImageView *monkeyView;
@property (strong, nonatomic) UIView *myBackgroundView;

@property (strong, nonatomic) UILabel *tipLabel;
@property (strong, nonatomic) UIButton *reloadButton;
@property (assign, nonatomic) kBlankPageType1 curType;
@property (copy, nonatomic) void(^reloadButtonBlock)(id sender);
@property (copy, nonatomic) void(^loadAndShowStatusBlock)(void);
@property (copy, nonatomic) void(^clickButtonBlock)(kBlankPageType1 curType);
- (void)configWithType:(kBlankPageType1)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError offsetY:(CGFloat)offsetY reloadButtonBlock:(void(^)(id sender))block;
@end
