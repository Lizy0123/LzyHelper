...
const static CGFloat kModalViewBtnHeight = 50;
const static CGFloat kModalViewBtnSpaceHeight = 1;
const static CGFloat kModalViewCornerRadius = 7;
const static CGFloat kModalViewMotionEffectExtent = 10.0;

@implementation MyModalView
CGFloat buttonHeight = 0;
CGFloat buttonSpacerHeight = 0;

@synthesize parentView, containerView, dialogView, buttonAction;
@synthesize delegate;
...
- (IBAction)myModalViewBtnAction:(id)sender{
    if (delegate != NULL) {
        [delegate myModalViewBtnAction:self clickedBtnAtIndex:[sender tag]];
    }

    if (buttonAction != NULL) {
        buttonAction(self, (int)[sender tag]);
    }
}

// Default button behaviour
- (void)myModalViewBtnAction:(MyModalView *)modalView clickedBtnAtIndex:(NSInteger)btnIndex{
    NSLog(@"Button Clicked! %d, %d", (int)btnIndex, (int)[modalView tag]);
    [self close];
}
...
- (UIView *)createContainerView{
    ...
    CGFloat cornerRadius = kModalViewCornerRadius;
...
- (void)addButtonsToView: (UIView *)container{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        [closeButton addTarget:self action:@selector(myModalViewBtnAction:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [closeButton.layer setCornerRadius:kModalViewCornerRadius];
...
- (CGSize)countScreenSize{
    if (buttonTitles!=NULL && [buttonTitles count] > 0) {
        buttonHeight       = kModalViewBtnHeight;
        buttonSpacerHeight = kModalViewBtnSpaceHeight;
    } else {
        buttonHeight = 0;
...
- (void)applyMotionEffects {
    ...
    horizontalEffect.minimumRelativeValue = @(-kModalViewMotionEffectExtent);
    horizontalEffect.maximumRelativeValue = @( kModalViewMotionEffectExtent);

    UIInterpolatingMotionEffect *verticalEffect = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:UIInterpolatingMotionEffectTypeTiltAlongVerticalAxis];
    verticalEffect.minimumRelativeValue = @(-kModalViewMotionEffectExtent);
    verticalEffect.maximumRelativeValue = @( kModalViewMotionEffectExtent);
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if ([touch.view isKindOfClass:[MyModalView class]]) {
        [self close];
    }
}
